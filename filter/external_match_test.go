...
func testExternalMatchFiles(t *testing.T, keepOnMatch bool, fields []string, wantErr []bool) {
	t.Helper()

	if len(fields) != len(wantErr) {
...
`
	...
	for i, v := range fields {
		...
		if (err != nil) != wantErr[i] {
			t.Errorf("Process() = %v, want err=%v; field=%v", err, wantErr[i], v)
		}
	}
}
...
func TestExternalMatchRefreshValues(t *testing.T) {
	...
	err = f.Process(&rec)
	if err != nil {
		t.Error("first call to Process() returned an error")
	}
	...
	err = f.Process(&rec)
	if err == nil {
		t.Error("second call to Process() returned nil, should have been an error")
	}
