...
func TestMetadataUrl(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			urlNotSet: true,
			dst:       "f1",
			want:      "", // the filter shoud clear "f1"
		},
		{
			...
			url:    nil,
			dst:    "f2",
			want:   "", // the filter shoud clear "f2"
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err = rec1.Parse([]byte(tt.record), bakerMetadata)
			if err != nil {
...
// TestMetadataUrlCopy checks that the byte slice with the URL that the filter set
// in the record field is a new one and it is not shared by consecutive records.
func TestMetadataUrlCopy(t *testing.T) {
	...
	// create two record with the same metadata
	rec1 := &baker.LogLine{FieldSeparator: ','}
	...
	f.Process(rec1, func(r baker.Record) {
		// the first record is modified
		b := r.Get(0)
		b[0] = 'x'
	})
	f.Process(rec2, func(r baker.Record) {
		// the second record should be not touched
		if r.Get(0)[0] == 'x' {
