...
var ExternalMatchDesc = baker.FilterDesc{
	...
	Help:   "Trigger errors for records which fields matches values read from a CSV (possibly periodically refreshed). CSV files can be compressed (gz or zstd) or not.",
...
func (f *ExternalMatch) Process(l baker.Record) error {
	f.mx.RLock()
